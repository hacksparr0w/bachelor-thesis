...
def main():
    ...
    energy_data = np.genfromtxt(
        output_dir / "energy_cutoff_convergence.csv",
        delimiter=" ",
        skip_header=1
    )

    force_data = np.genfromtxt(
        output_dir / "force_cutoff_convergence.csv",
        delimiter=" ",
        skip_header=1
    )

    output_file = (
        output_dir / f"cutoff_convergence.jpg"
    )
...
def plot(energy_data, force_data, output_file):
    ...
    fig.update_xaxes(
        title_text="Enegy Cutoff [Ry]",
        col=1,
        row=2
    )

    fig.update_yaxes(
        title_text="Total Energy [eV]",
        tickformat=".5f",
        showgrid=False,
        linewidth=1,
        linecolor="black",
